...
if ( strpos( dt_get_url_path(), 'zume_app' ) !== false || dt_is_rest() ){
    Zume_Simulator_Public_Heatmap_Churches::instance();
}

class Zume_Simulator_Public_Heatmap_Churches extends DT_Magic_Url_Base
{
    ...
    public function _header(){
        Zume_Simulator_App_Heatmap::_header();
    }

    public static function _wp_enqueue_scripts(){
        Zume_Simulator_App_Heatmap::_wp_enqueue_scripts();
    }
    ...
    public function endpoint( WP_REST_Request $request ) {
        ...
        switch ( $action ) {
            case 'self':
                return Zume_Simulator_App_Heatmap::get_self( $params['grid_id'], $this->global_div, $this->us_div );
            case 'a3':
            case 'a2':
            case 'a1':
            case 'a0':
            case 'world':
                $list = Zume_Simulator_App_Heatmap::query_church_grid_totals( $action );
                return Zume_Simulator_App_Heatmap::endpoint_get_level( $params['grid_id'], $action, $list, $this->global_div, $this->us_div );
            case 'activity_data':
                $grid_id = sanitize_text_field( wp_unslash( $params['grid_id'] ) );
                $offset = sanitize_text_field( wp_unslash( $params['offset'] ) );
                return Zume_Simulator_App_Heatmap::query_activity_data( $grid_id, $offset );
            case 'grid_data':
                $grid_totals = Zume_Simulator_App_Heatmap::query_church_grid_totals();
                return Zume_Simulator_App_Heatmap::_initial_polygon_value_list( $grid_totals, $this->global_div, $this->us_div );
            default:
